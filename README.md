# MCP-TODO

A beginner-friendly Todo application built to teach the **Model Context Protocol (MCP)** — a standard way for AI models and clients to communicate with tools and services.

---

## Summary

- The **server** hosts tools like `add`, `update`, `delete`, and `list`
- The **client** connects to the server and calls those tools by name
- The **UI** uses the client to perform all task operations

---

## Runtimes
- [Python](python/README.md)